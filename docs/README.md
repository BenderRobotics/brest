...
Most of the customization is done via theme bult-in functionalities that can be controlled in `.\conf.py`.
...
- Title
    ...
    - The string is explicitely set (`conf.py` - `#PY-002`).
...
- Logo
    ...
    - It can be enabled by setting `html_logo` variable (`conf.py` - `#PY-005`).
        - This is commented out now as there is no official logo ready yet.
...
- Version
    ...
    - It's visibility is explicitely set (`brest.css` `#CSS-005`).
    - Space between Logo and Version is adjusted (`brest.css`- `#CSS-004`).
- Navigation menu
    - Navigation menu is located on the left side.
        - The background color is explicitely set (`conf.py` - `#PY-004`).
...
- Breadcrumbs
    ...
    - There is a "home" icon for returning back to the *index* of the documentation.
        - This icon's size is explicitely set (`brest.css` - `#CSS-007`).
- Other
    ...
    - The template enables to show copyright text.
        - The text is explicitely set (`conf.py` - `#PY-004`).
